        ...
        for (CItem* i : m_items)
            if (!i->m_hidden || (i->m_hidden && print_hidden)) {
                if (typeid(*i) == typeid(CFolder))
                    dynamic_cast<CFolder*>(i)->PrintHelper(print_hidden);
                else
    ...
    for (int i = 0; i < m_items.size(); ++i) {
        if (!m_items[i]->m_read_only) {
            ...
            if (typeid(*m_items[i]) == typeid(CFolder)) {
                CItem* res = dynamic_cast<CFolder*>(m_items[i])->RemoveByName(name);
                if (res)
                    return res;
            }
        }
